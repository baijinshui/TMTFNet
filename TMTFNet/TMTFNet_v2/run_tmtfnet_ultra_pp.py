"""TMTFNet-Ultra++ : Ultra with the same patch granularity as PatchTST baseline
(patch_len=8, stride=4) and larger hidden dim d=128. Targets a final push past
PatchTST 87.53% on PAMAP2 cross-subject HAR."""
...
def run_pamap2(out_dir, max_epochs=80, mixup=0.2):
    ...
    accs = []
    for seed in SEEDS:
        path = out_dir / f"TMTFNet_ultraPP_seed{seed}.json"
        ...
        model = build_model(
            ...
            d_model=128, n_heads=8, n_enc_layers=2,    # larger hidden
            modality_dropout=0.1, n_classes=bundle.n_classes,
            dropout=0.1, seq_len=171,
            patch_len=8, patch_stride=4,               # finer patches
        )
        ...
        result.update({"seed": int(seed), "model_name": "TMTFNet_ultraPP", "n_params": int(n_params),
...
if __name__ == "__main__":
    ...
    run_pamap2(REPO / "results" / "rebuttal_exp1c_har_pamap2_ultraPP")
