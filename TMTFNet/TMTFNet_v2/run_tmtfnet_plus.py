"""TMTFNetPlus on UCI HAR + PAMAP2 cross-subject (5 seeds each).

Goal: beat PatchTST (94.18% on UCI HAR, 87.53% on PAMAP2) by adding a
parallel patch-token branch to TMTFNet.
"""
...
def run_har(out_dir, max_epochs=100):
    out_dir.mkdir(parents=True, exist_ok=True)
    accs = []
    for seed in SEEDS:
        path = out_dir / f"TMTFNetPlus_seed{seed}.json"
        ...
        model = build_model(
            ...
            patch_len=8, patch_stride=4, n_patch_layers=2,
        )
        ...
        result = train_and_evaluate(
            ...
            max_epochs=max_epochs, patience=15, label_smoothing=0.1,
        )
        result.update({"seed": int(seed), "model_name": "TMTFNetPlus", "n_params": int(n_params),
...
def run_pamap2(out_dir, max_epochs=60):
    ...
    for seed in SEEDS:
        ...
        model = build_model(
            ...
            patch_len=16, patch_stride=8, n_patch_layers=2,  # patches sized for 171-len windows
        )
        n_params = count_parameters(model)
        t0 = time.time()
        result = train_and_evaluate(
...
if __name__ == "__main__":
    ...
    run_har(REPO / "results" / "rebuttal_exp1a_har_uci_plus")
    run_pamap2(REPO / "results" / "rebuttal_exp1c_har_pamap2_plus")
